...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (loc.u_time) ctx.uniform1f(loc.u_time, currentTime);
      if (loc.u_resolution) ctx.uniform2f(loc.u_resolution, width, height);
      if (loc.u_colorBack) ctx.uniform4fv(loc.u_colorBack, hexToRgba(colorBack));
      if (loc.u_colorFront) ctx.uniform4fv(loc.u_colorFront, hexToRgba(colorFront));
      if (loc.u_shape) ctx.uniform1f(loc.u_shape, DitheringShapes[shape]);
      if (loc.u_type) ctx.uniform1f(loc.u_type, DitheringTypes[type]);
      if (loc.u_pxSize) ctx.uniform1f(loc.u_pxSize, pxSize);
